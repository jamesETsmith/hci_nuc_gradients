...
noon_files = [
    f"../../dz/{d}/gen_uno/_data/{d}_DZ_100e_100o_eps1=0.0001_vhci_noons.txt"
    for d in states
]
logfiles = [
    f"../../dz/{d}/gen_uno/_logs/{d}_DZ_100e_100o_eps1=0.0001_vhci.out" for d in states
]
...
df_noons = pd.DataFrame(
    np.array([np.loadtxt(nf) for nf in noon_files]).T, columns=states
)

print(df_noons.columns)
df_noons.to_csv("noons.csv")
df.to_html("uno_results_full.html", index=False)
df.drop(
    columns=[
        "Energy (Ha)",
        "Fe 3dxy pop",
...
).to_html("uno_results.html", index=False)
