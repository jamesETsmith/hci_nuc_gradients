...
outputbase = f"TSG_{nelecas}e_{ncas}o_eps1={eps1}"
...
mol.basis = "ccpvdz"
mol.verbose = 6
mol.output = f"_logs/_{outputbase}.out"
mol.max_memory = 100000  # in MB
mol.build()


mf = scf.UHF(mol).newton()
mf.__dict__.update(chkfile.load("_chk/uhf_dz_TSG.chk", "scf"))

#
# HCISCF Calculation
#
mc = shci.SHCISCF(mf, ncas, nelecas).density_fit()
mc.chkfile = f"_chk/_{outputbase}.chk"
mc.natorb = True
mc.fcisolver.sweep_iter = [0, 3, 6, 9]
mc.fcisolver.sweep_epsilon = [eps1 * 10, eps1 * 5, eps1 * 2, eps1]
mc.fcisolver.mpiprefix = f"mpirun -np {nprocs}"
mc.fcisolver.scratchDirectory = f"{scratchdir}/fepdi/triplet/{outputbase}"
...
uhf_NOs = np.load("_chk/uhf_dz_TSG_NOs.npy")
mc.conv_tol = 1e-4
mc.mc1step(uhf_NOs)

# Save Data
np.savetxt(f"_data/{outputbase}_noons.txt", mc.mo_occ)
molden.from_mo(mol, f"_molden/_{outputbase}.molden", mc.mo_coeff)
