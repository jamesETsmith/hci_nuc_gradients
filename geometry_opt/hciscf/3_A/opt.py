...
#
nelecas = ncas
outputbase = f"3_A_{nelecas}e_{ncas}o_eps1={eps1}"
scratchdir = os.environ.get("TMP")
nprocs = os.environ.get("OMP_NUM_THREADS")  # This is used for MPI in Dice

uhf_chkfile = "../gen_uno/_chk/uhf_3_A.chk"
uno_chkfile = "../gen_uno/_chk/3_A_100e_100o_eps1=0.001.chk"
...
mc = shci.SHCISCF(mf, ncas, (nelecas // 2 + 1, nelecas // 2 - 1)).density_fit()
...
mc = shci.SHCISCF(mf2, ncas, (nelecas // 2 + 1, nelecas // 2 - 1)).density_fit()
mc.chkfile = f"_chk/{outputbase}.chk"
mc.fcisolver.sweep_iter = [0, 9, 15]
mc.fcisolver.sweep_epsilon = [eps1] * 3
mc.fcisolver.mpiprefix = f"mpirun -np {nprocs}"
