...
def make_n2(hi, mo0=None, dm0=None):
    mol = gto.Mole()
    # In Angstroms
    mol.atom = [["N", np.array([0, 0, 0])], ["N", np.array([0, 0, 1.0 + hi])]]
    mol.basis = "ccpvdz"
    # mol.unit = "Angstrom"
    mol.verbose = 4
    mol.symmetry = True
    ...
    mc.fcisolver.epsilon2 = 1e-12  # 1e-12
    mc.conv_tol = 1e-12
...
if __name__ == "__main__":
    ...
    fd_energies = [make_n2(h_i, mo0.copy(), dm0.copy())[0].e_tot for h_i in hlist]
    ...
    print(abs(pyscf_grad[1][2] - data["five_pt"]))
