...
vhciscf = [np.load("_data/vhciscf_{:.1e}.npy".format(eps1)) for eps1 in [1e-3, 5e-4, 1e-4, 1e-5, 1e-6]]
vhciscf_err = [rms_error(shci,casscf) for shci in vhciscf]

# vhciscf_aa = [np.load("_data/vhciscf_aa_{:.1e}.npy".format(eps1)) for eps1 in [1e-3, 5e-4, 1e-4]]
...
# plt.loglog([1e-3, 5e-4, 1e-4], vhciscf_aa_err, "o-", label=label[1])
